...
BOOL WINAPI ConsoleHandler(DWORD signal) {
	switch (signal) {
		...
		case CTRL_SHUTDOWN_EVENT:
			...
			Log(LogLevel::Warn, "Stop signal received, shutting down...");
			return TRUE;
		default:
			return FALSE;
	}
...
int main(int argc, char** argv) {
	...
	do {
		...
		adapter = WintunOpenAdapter(wAdapterName.c_str());
		if (adapter == nullptr) {
			Log(LogLevel::Warn, "Adapter not found, creating new adapter...");
			adapter = WintunCreateAdapter(wAdapterName.c_str(), wTunnelType.c_str(), nullptr);
			if (adapter == nullptr) {
		...
		session = WintunStartSession(adapter, kWintunRingCapacity);
		if (session == nullptr) {
		...
		if (bind(sock, reinterpret_cast<const sockaddr*>(&localAddr), sizeof(localAddr)) != 0) {
			Log(LogLevel::Error, "bind failed. err=" + std::to_string(WSAGetLastError()));
			break;
		}
		...
		std::thread netToTun([&]() {
			...
			while (g_running.load()) {
				...
				BYTE* out = WintunAllocateSendPacket(session, recvLen);
				if (!out) {
