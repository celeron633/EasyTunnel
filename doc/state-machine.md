...
所有回退都要求 IPv4 阶段已经通过 `PEER` 匹配到明确的目标 Peer。没有
`matchedPeerId` 时，不会尝试 IPv6 或 relay。
...
stateDiagram-v2
    ...
    WaitingPeer --> ExactPunch: 收到合法 PEER
    WaitingPeer --> Failed: 首次响应或选定 Peer 超时
    WaitingPeer --> Stopped: Stop

    ExactPunch --> DataPathReady: PUNCH 或 PUNCH_ACK
    ExactPunch --> Nat4: 2 秒未成功且 NAT4 已启用
    ExactPunch --> TryIpv6: punch_timeout 且启用 IPv6
    ExactPunch --> TryRelay: punch_timeout 且未启用 IPv6、已启用 Relay
    ExactPunch --> Failed: 无可用回退

    Nat4 --> DataPathReady: 任一 socket 收到合法 Peer PUNCH
    Nat4 --> TryIpv6: 总 punch_timeout 且启用 IPv6
    Nat4 --> TryRelay: 总 punch_timeout 且未启用 IPv6、已启用 Relay
    Nat4 --> Failed: 无可用回退

    TryIpv6 --> DataPathReady: V6_PUNCH / ACK 确认
    TryIpv6 --> TryRelay: IPv6 失败且启用 Relay
    TryIpv6 --> Failed: Relay 未启用

    TryRelay --> DataPathReady: 收到合法 RELAY_READY
    TryRelay --> Failed: 拒绝或 punch_timeout
...
当 `nat4_source_port_count=0` 时，不存在图中的 2 秒切换点，精确端口 Punch 会持续到
`punch_timeout`。IPv6 与 relay 都是新一轮独立超时，不复用已经到期的 IPv4 deadline。
...
sequenceDiagram
    ...
    A->>S: REG(room, A, token)
    B->>S: REG(room, B, token)
    S-->>A: REGISTERED
    S-->>B: REGISTERED
    A->>S: CONNECT(room, A, B, token)
    S-->>A: PEER(B IPv4 endpoint)
    S-->>B: PEER(A IPv4 endpoint)
...
| 精确 Punch | NAT4 启用时 2 秒未确认 | 进入 NAT4 |
| IPv4/NAT4 总阶段 | `punch_timeout` | IPv6、Relay 或 `Error` |
| IPv6 Fallback | `ipv6_fallback_timeout` | Relay 或 `Error` |
| IPv4 Relay 协商 | 新一轮 `punch_timeout` | `Error` |
