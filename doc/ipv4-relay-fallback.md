...
IPv4 Relay Fallback 是 IPv4 精确端口打洞、NAT4 socket 池和可选 IPv6 Fallback
...
```ini
ipv4_relay_fallback_enabled=true
```
...
- Console：在 `tunnel.conf` 中设置 `ipv4_relay_fallback_enabled`；
- TUI：Settings → IPv4 Relay Fallback；
- GUI：Settings → IPv4 Relay Fallback；
- 会合服务器 TUI：Config → IPv4 relay，可配置开关及端口范围；Dashboard 显示活动
  relay 数、已转发数据报和字节数。

## 常见错误

- `ipv4-relay-disabled`：服务器未启用 relay；
