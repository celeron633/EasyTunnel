...
只有 IPv4 阶段已经匹配到目标 Peer，且双方上报的模式能力都包含 `ipv4_relay` 时才允许
进入 relay，不能使用 relay 绕过原有 room、Peer ID 和 `auth_token` 校验。双方模式配置
可以不同；会合服务器会按连接发起方的顺序取交集，使两端同时进入 relay。Relay 协商
使用新一轮 `punch_timeout` 作为等待时限。
...
- `Peer ... does not support any enabled traversal mode`：双方上报的模式能力没有交集，
  在创建 Relay 会话前已经失败；
- `IPv4 relay fallback timed out`：数据端口未放行、Relay 握手包丢失，或服务器端会话
  提前退出；
